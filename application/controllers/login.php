...
class login extends CI_Controller {
    ...
    public function index() {
        ...
        $data['title'] = "Elevate Web Control Panel";
        if ($this->form_validation->run() != FALSE) {
            $email = $this->input->post("email");
            $password = md5($this->input->post("password"));
            $result = $this->admindbmodel->fetch_data_two_con($email, $password, 'email', 'password', $this->tbl_name1);
            if (empty($result)) {
                $data['errorMsg'] = "Email or  Password Does not Match";
